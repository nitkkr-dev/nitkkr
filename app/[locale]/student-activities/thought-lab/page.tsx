...
export default async function ThoughtLab({
  ...
  const facultyMembers =
    text.ThoughtLab.secretaries.faculty_secretaries.members.map(
  ...
  const studentMembers =
    text.ThoughtLab.secretaries.student_secretaries.members.map(
      (member: SecretaryMember, idx: number) => ({
  ...
  return (
    <>
        ...
        <section className="mt-4 flex rounded bg-shade-light p-2">
          <ul>
            <li className="p-3">
              ...
              {text.ThoughtLab.vision.points.map(
            ...
            <li className="p-3">
              ...
              {text.ThoughtLab.mission.points.map(
                (item: string, index: number) => (
                  <p className="mb-1" key={index}>
                    {item}
                  </p>
      ...
      <section className="container">
        ...
        />
        <ul className="list-disc space-y-2 pl-5">
          {text.ThoughtLab.benefits.points.map(
            (item: string, index: number) => (
              <li
                key={index}
                className="text-[16px] text-[20px] font-normal leading-snug md:text-[20px] md:text-[20px]"
      ...
      <section className="container">
        <Heading
          glyphDirection="ltr"
          heading="h3"
          text={text.ThoughtLab.gallery.heading}
        />
        <Gallery
          base={getS3Url()}
