...
export async function populate() {
  ...
  //   return date;
  // };
  // const studentCsv = fs.readFileSync('student-data.tsv', 'utf-8').split('\n');
  // const studentHeaders = studentCsv[0].split('\t');
  // for (let i = 1; i < studentCsv.length; i++) {
  //   const studentData = convertToData(studentCsv[i], studentHeaders);
  //   console.log(studentData['Roll no']);
  //   const phoneNumber = extractPhoneNumber(studentData['Contact no.']);
  //   const { id } = await db
  //     .insert(schemas.persons)
  //     .values({
  //       type: 'student',
  //       name: studentData['Student Name'],
  //       email: studentData["Institute's Email ID"],
  //       countryCode: phoneNumber?.countryCode,
  //       telephone: phoneNumber?.number ?? '',
  //       sex: studentData.Gender as 'M' | 'F' | 'O',
  //       dateOfBirth: studentData['Date of Birth']
  //         ? adjustDate(studentData['Date of Birth'])
  //         : adjustDate('2099/01/01'),
  //       createdOn: studentData['Date of Admission']
  //         ? adjustDate(studentData['Date of Admission'])
  //         : adjustDate('2099/01/01'),
  //     })
  //     .returning({ id: schemas.persons.id })
  //     .then((res) => res[0]);
  //   const admissionCategory = studentData['Admission Category'];
  //   const fatherPhoneNumber = extractPhoneNumber(
  //     studentData["Father's Contact no."]
  //   );
  //   const { studentId } = await db
  //     .insert(schemas.students)
  //     .values({
  //       id: id,
  //       rollNumber: studentData['Roll no'],
  //       fathersName: studentData["Father's Name"],
  //       fathersTelephoneCountryCode: fatherPhoneNumber?.countryCode ?? '',
  //       fathersTelephone: fatherPhoneNumber?.number ?? '',
  //       fathersEmail: studentData["Father's Email"] || '',
  //       candidateCategory: admissionCategory?.split('(')[0] as
  //         | 'GEN-EWS'
  //         | 'OBC-NCL'
  //         | 'OPEN'
  //         | 'SC'
  //         | 'ST',
  //       isPwd: admissionCategory?.includes('PwD'),
  //       admissionCategory: ['DASA', 'MEA', 'OPEN', 'SII'].includes(
  //         admissionCategory
  //       )
  //         ? (admissionCategory as 'DASA' | 'MEA' | 'OPEN' | 'SII')
  //         : 'OPEN',
  //     })
  //     .returning({ studentId: schemas.students.id })
  //     .then((res) => res[0]);
  //   const { majorId } = await db
  //     .select({ majorId: schemas.majors.id })
  //     .from(schemas.majors)
  //     .where(
  //       and(
  //         eq(
  //           schemas.majors.degree,
  //           studentData.Program as
  //             | 'B. Tech.'
  //             | 'M. Tech.'
  //             | 'MCA'
  //             | 'MBA'
  ...
  //             | 'Ph. D.'
  //         ),
  //         eq(schemas.majors.name, studentData.Branch)
  //       )
  //     )
  //     .then((res) => res[0]);
  //   await db.insert(schemas.studentAcademicDetails).values({
  //     id: studentId,
  //     batch: 20 + Number(studentData['Roll no'].slice(1, 3)),
  //     section: ' ',
  //     subSection: 0,
  //     currentSemester: 0,
  //     sgpa: 0,
